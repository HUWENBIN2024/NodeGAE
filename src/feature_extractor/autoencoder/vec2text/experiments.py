...
# from utils import MockEmbedder, dataset_map_multi_worker
...
class Experiment(abc.ABC):
    ...
    def train(self) -> Dict:
        ...
        trainer.model.encoder_decoder.config.decoder_start_token_id = 0
        ...
        torch.save(trainer.model.graph_model, os.path.join(training_args.output_dir, "graph_model.bin"))
    ...
    def _get_checkpoint(self) -> Optional[str]:
        ...
        # training_args.output_dir == ./saves/gtr-arxiv-2
        if (
    ...
    def _load_train_dataset_uncached(
        ...
        # raw_datasets = torch.load('../../data_with_1024.pt')
        # raw_datasets["train"] = raw_datasets["train"].rename_column("label", "idx")
        # raw_datasets["validation"] = raw_datasets["test"].rename_column("label", "idx")
        # raw_datasets["test"] = raw_datasets["test"].rename_column("label", "idx")
        # Remove extra features except for 'frozen_embeddings' which could be embeddings
        # saved to disk.
        column_names = list(raw_datasets["train"].features)
        ALLOWED_COLUMN_NAMES = {"idx"}
