...
class CorrectorEncoderModel(transformers.PreTrainedModel):
    ...
    # TODO: make the graph method for the corrector encoder
    class linear_projection(nn.Module):
        def __init__(self, in_num=256, out_num=768):
    ...
    def get_encoder_embedding_g(
        ...
        projection = linear_projection(in_num=embedding.shape[1], out_num=768)
        embedding = projection(embedding)
        
        
        hypothesis_words = T5Tokenizer.from_pretrained("t5-base").decode(hypothesis_input_ids, skip_special_tokens=False)
        # TODO make the hypothesis_words into the corresponding position
        ##################################################################
        
        ##################################################################
        
        # TODO run the graphSAGE again
        ##################################################################
        
        ##################################################################
        
        # TODO take the hypothesis_embedding from the graphSAGE
        ##################################################################
