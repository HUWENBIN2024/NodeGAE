...
os.environ["VEC2TEXT_CACHE"]='../../../../.cache_autoencoder' # modified
...
class Corrector(BaseTrainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.emb = torch.load('/data/hjingaa/github/geia_graph/exchange_files/graph_emb.pt',map_location=device)
        self.graph_data = torch.load('/data/hjingaa/github/geia_graph/exchange_files/title_emb_data_x_for_SAGE.pt', map_location=device)

        self.graph_model = SAGE_embedding(768, 1024, 40, 3, 0.5).to(device)
        self.graph_model.load_state_dict(torch.load('/data/hjingaa/github/geia_graph/exchange_files/sage_model_arxiv_1024.pt', map_location=device))
    ...
    def _get_hypothesis_uncached(self, inputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        with torch.no_grad():
            dim = 0
            for i in inputs["idx"].tolist():
                # self.emb.requires_grad_(False)
                self.emb[i] = hypothesis_embedding[dim]
                # self.emb.requires_grad_(True)
                dim += 1
                
            # self.emb.requires_grad_(False)
            self.emb = self.graph_model(self.emb, self.graph_data.adj_t)
            # self.emb.requires_grad_(True)
            
            dim = 0
            for i in inputs["idx"].tolist():
                # hypothesis_embedding.requires_grad_(False)
                hypothesis_embedding[dim] = self.emb[i]
                # hypothesis_embedding.requires_grad_(True)
                dim += 1
        return (
            frozen_embeddings,
            hypothesis_input_ids,
