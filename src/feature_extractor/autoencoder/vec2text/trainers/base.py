...
def infonce_loss(query, positive, negatives=None, temperature=0.5):
    """
    Computes the InfoNCE loss.
    
    Args:
        query (torch.Tensor): The query tensor of shape (batch_size, embedding_dim).
        positive (torch.Tensor): The positive tensor of shape (batch_size, embedding_dim).
        negatives (torch.Tensor): The negative tensors of shape (batch_size, num_negatives, embedding_dim).
    ...
    sim_matrix = torch.mm(query, query.T)
    neg_matrix = sim_matrix[~torch.eye(batch_size).bool()].reshape(batch_size, batch_size-1) # (bs, bs-1) 
    
    # Compute the logits
    # logits = torch.cat([torch.sum(query * positive, dim=1, keepdim=True), # 64, 1
    #                    torch.sum(query[:, None, :] * negatives, dim=-1)], dim=1) # 64, 10
    logits = torch.cat([torch.sum(query * positive, dim=1, keepdim=True), # (bs, 1)
                       neg_matrix], dim=1) # (bs, bs)
...
class BaseTrainer(transformers.Trainer):
    ...
    def _get_decoded_sequences(
        ...
        for step, inputs in enumerate(
            ...
            gen_kwargs["max_length"] = max_length
            gen_kwargs["node_idx"] = inputs["node_idx"]
            with torch.no_grad():
    ...
    def eval_generation_metrics(
        ...
        metrics = {**num_tokens_metrics, **bleu_result}
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if return_outputs or not self.args.use_infonce_loss:
            return super().compute_loss(model, inputs, return_outputs)

        # print("Into the loss function\n")
        loss_sentence = super().compute_loss(model, inputs, return_outputs) # TODO: finish this loss function
        neighbour_loss_1st_order = infonce_loss(self.model.query , self.model.positive_samples_1st_order, temperature=0.5)
        neighbour_loss_2nd_order = infonce_loss(self.model.query , self.model.positive_samples_2st_order, temperature=0.5)

        return loss_sentence + (neighbour_loss_1st_order + 0.1 * neighbour_loss_2nd_order) * self.args.infonce_loss_weight