...
class OgbnProductsWithText(OgbWithText):
    def __init__(
        self, root="data", transform=None, pre_transform=None, tokenizer='sentence-transformers/sentence-t5-base', tokenize=True
    ):
    ...
    def _mapping_and_tokenizing(self):
        ...
        for i in tqdm(range(0, len(df), truncated_size)):
            ...
            _encodings = self.tokenizer(text_list[i:j], padding=True, truncation=True, return_tensors="pt", max_length=256)
