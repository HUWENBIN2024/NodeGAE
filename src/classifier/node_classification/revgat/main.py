...
def load_data(dataset, args):
    ...
    data = DglNodePropPredDataset(name=dataset, root="datasets")
    ...
    if args.use_bert_x:
        graph.ndata["feat"] = torch.load(args.bert_x_dir, map_location='cpu')
        logger.warning(
    ...
    # TODO
        ...
        preds = []
        with open(f"src/classifier/node_classification/revgat/gpt_preds/ogbn-arxiv.csv", "r") as file:
